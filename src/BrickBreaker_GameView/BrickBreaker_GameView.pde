 ...
 * File : SerialGame
 * Author : Frank Fang
 * Processing part : build the game and draw GUI
 */
 
/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/134612*@* */
...
void setup()
{
  ...
  int portNumber = 1;  // if not work, try 0~3
  String portName = Serial.list()[portNumber];
  // print all serial names
  for (int i = 0; i < 4; i++)
  {
...
void draw()
{
  ...
  if ((score == total || lives <= 0) && buttonPressed)
  {
    resetGame();
  } 
...
void readData() {
    ...
    if (val == 's') {
      posX = myPort.read();
      // different speed for different input
      if (posX < 50) {
        direct = -2;
      } else if (50 <= posX && posX < 100){
        direct = -1;
      } else if (100 <= posX && posX < 150){
        direct = 0;
      } else if (150 <= posX && posX < 200){
        direct = 1;
      } else {
        direct = 2;
      }
